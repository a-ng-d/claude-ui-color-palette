...
```
Frame "Palette Preview"          ← root, Auto Layout vertical, gap 24, padding 24, fill #F5F5F5
  Frame "<theme name>"           ← Auto Layout vertical, gap 4, padding 16, fill #FFFFFF, corner 8
    Text "<theme name>"          ← 14px, bold, #111111
    Frame "<color name>"         ← Auto Layout horizontal, gap 0, align center
...
| Fill | shade hex |
| Auto Layout | vertical, align center, gap 4, padding 8 |
...
## Figma MCP API sequence

1. Create root frame with vertical Auto Layout
2. For each theme:
   a. Create a theme frame with vertical Auto Layout, white fill
   b. Add theme name text
   c. For each color in that theme:
      - Create a row frame with horizontal Auto Layout, no fill
...
3. Place root frame at a clear position on the current page (offset from existing content)
